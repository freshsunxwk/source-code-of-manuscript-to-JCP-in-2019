...
contrast.matrix<-makeContrasts(paste0(unique(group_list),collapse = "-"),
                               levels = design)
contrast.matrix<-makeContrasts("LPS-WT",
                               levels = design)
contrast.matrix<-makeContrasts("TAA-Control",
                               levels = design)

contrast.matrix 
##这个矩阵声明，我们要把72h处理组跟未处理的细胞系进行差异分析比较
save(exprSet,design,contrast.matrix,file="zhongjianwenjian.Rdata")
...
iiii = deg(exprSet,design,contrast.matrix)
...
save(exprSet,group_list,nrDEG,DEG, 
     file='GSE8021_DEG.Rdata')
