...
tiff("step2-beta-value.tif",width=80, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
# Plot the results:
##sizeGrWindow(9, 5)
par(mfrow = c(1,2));
cex1 = 0.9;
...
#tiff("Clustering-of-module-eigengenes.tif",width=50, height=60, units='mm',res=1200,compression='lzw')
sizeGrWindow(7, 6)
...
table(mergedColors)
#mergedColors
#black      blue     brown     green      pink       red turquoise 
#	88      4204       379       316        82        95      4836
...
tiff("Dynamic-Tree-Cut-Merged-dynamic.tif",width=90, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
...
tiff("sample-subtype-cluster.tif",width=100, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
...
moduleTraitCor_noFP <- cor(mergedMEs, datTraits[,1:3], use = "p");
...
tiff("step5-Module-trait-relationships.tif",width=60, height=100, units='mm',res=1200,compression='lzw',pointsize=6)
...
sizeGrWindow(8,7)
par(mfrow = c(1,1))
plotModuleSignificance(GeneSignificance,mergedColors,ylim=c(-0,0.2))

#这里可以根据：KME (eigengene connectivity)值来筛选hub基因：
#Hub genes are those that show most connections in the network as indicated by 
#their high KME (eigengene connectivity) value
#代码部分很简单，输出所有的基因与模块的KME矩阵，
#然后筛选每个模块中kme最大的前几个基因就是hub基因：
#mergedMEs是经过相似模块合并之后的最终模块对应的模块特征基因：
MEs = mergedMEs
#输出结果：
datKME=signedKME(datExpr, mergedMEs, outputColumnName="kME_MM.")	
colnames(datKME)

#kME_MM.turquoise   xuan zhe mo kuai 
...
#kME_MM.turquoise
FilterGenes_spe = ((GeneSignificance> 0.4) & (abs(datKME$kME_MM.turquoise)>0.8) & (NS1$q.Weighted < 0.01) ) 
# 根据 基因与指定性状的直接相关性(biserial.cor)，模块身份，和加权相关性 筛选基因：
table(FilterGenes_spe)
# 找到满足上面条件的基因：
trait_hubGenes_spe <- colnames(datExpr)[FilterGenes_spe]

write.csv(trait_hubGenes_spe,"trait_hubGenes_spe-02-08-001-599.csv")
write.csv(NS1,"NS1.csv")
write.csv(geneModuleMembership,"geneModuleMembershi.csv")
write.csv(geneTraitSignificance,"geneTraitSignificance.csv")
fff=cbind(geneTraitSignificance,GSPvalue)
write.csv(fff,"geneTraitSignificance_GSPvalue.csv")
qqq=merge(trait_hubGenes_spe,datKME$kME_MM.turquoise,NS1$q.Weighted)#
write.csv(qqq,"san_ge_tiao_jian_biao.csv")
另外两只内置的方法，寻找
...
# tiff("step4-genes-modules.tif",width=80, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
...
if(T){
  ...
  tiff("step6-Module_membership-gene_significance-of-MEgrey.tif",width=80, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
  #sizeGrWindow(7, 7);
  par(mfrow = c(1,1));
  verboseScatterplot(abs(geneModuleMembership[moduleGenes, column]),
                     abs(geneTraitSignificance[moduleGenes, 1]),
                     xlab = paste("Module Membership in", module, "module"),
                     ylab = "Gene significance for GPD",
...
if(T){
  ...
  tiff("step7-Network-heatmap.tif",width=80, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
  ...
  par(cex = 0.9)
  tiff("step7-Eigengene-dendrogram.tif",width=80, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
  ...
  par(cex = 1.0)
  ## 模块的进化树
  tiff("step7-Eigengene-dendrogram-hclust.tif",width=80, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
  ...
  tiff("step7-Eigengene-adjacency-heatmap.tif",width=80, height=60, units='mm',res=1200,compression='lzw',pointsize=6)
...
if(T){
  ...
  gene= modProbes
  write.csv(gene,"greenyellow.csv")
